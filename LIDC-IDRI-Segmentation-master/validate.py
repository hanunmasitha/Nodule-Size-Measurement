...
def main():
    ...
    # Data loading code
    IMAGE_DIR = r'D:\Hanun\Dataset\Image'
    ...
    OUTPUT_MASK_DIR = r'D:\Hanun\Dataset\Segmentation Output\{}'.format(NAME)
    ...
    clean_meta = pd.read_csv(r'D:\Hanun\Dataset\Preprocessing Output\Meta\Fix\clean_meta.csv')
    # Get train/test label from clean_meta.csv
    clean_meta['original_image']= clean_meta['original_image'].apply(lambda x: x +'.npy')
    clean_meta['mask_image'] = clean_meta['mask_image'].apply(lambda x: x +'.npy')
    ...
    CLEAN_OUTPUT_MASK_DIR = r'D:\Hanun\Dataset\Segmentation Output\{}'.format(CLEAN_NAME)
